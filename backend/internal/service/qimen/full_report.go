...
// FullReportGenerator produces qimen full reports via DeepSeek with template fallback.
...
func (g *FullReportGenerator) Generate(
	...
	if g != nil && g.deepseek != nil && g.deepseek.enabled() {
		...
		if aiErr == nil {
			log.Printf("[ai] analysis_id=%d module=qimen provider=deepseek model=%s duration_ms=%d success=true",
				analysisID, modelName(g.cfg), duration)
			return aiContent, model.AIProviderDeepSeek, nil
		}
		log.Printf("[ai] analysis_id=%d module=qimen provider=deepseek model=%s duration_ms=%d success=false error=%q",
