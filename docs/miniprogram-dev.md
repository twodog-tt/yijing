...
## 34. Phase MINIAPP-UX1：加载态 / 错误态 / 空状态与防重复提交优化

**范围：** 小程序问事起卦、八字简析、奇门问事、历史记录、三类结果页与分享长图组件；**不改** backend / Web / SQL / deploy / `.env*` / 默认算法。

**变化摘要：**

- 统一网络错误提示为「网络似乎不太稳定，请稍后再试」，减少原始网络 / 后端错误直接暴露。
- 结果页加载失败统一引导用户回到历史记录重新进入；创建成功但结果页打开失败时不重复提交，提示从历史记录找回。
- 八字 / 奇门最近记录、历史记录加载失败使用可重试错误态；历史页空状态继续按全部 / 问事 / 八字 / 奇门提供入口。
- 历史页删除 loading 改为按记录类型 + ID 锁定，避免不同模块同 ID 记录同时进入删除态。
- 分享长图保存失败统一提示「保存失败，请检查相册权限」，保留打开设置授权流程。

**不变项：**

- 八字默认 `bazi-simple-v1`，奇门默认 `qimen-simple-v1`。
- 普通用户不展示算法选择；内部灰度展示边界不变。
- 不新增广告 / 支付 / 微信登录。
- 不修改 API payload、数据库、后端报告生成逻辑。

**验收：**

- [x] 问事 / 八字 / 奇门提交按钮在请求中保持 disabled / loading，避免重复创建。
- [x] 网络失败、记录打不开、报告生成失败、相册保存失败均有明确用户提示。
- [x] 历史记录空状态和筛选入口保留。
- [x] 长图预览 / 保存流程不展示完整原问题、完整报告全文、session_key 或 payload。
...
## 41. Phase UI1：小程序首页、结果页、长图视觉层级与可读性优化

**范围：** 小程序首页、问事结果页、问事分享长图组件与 Canvas 绘制；**不改** backend / frontend / SQL / deploy / `.env*` / 默认算法，不新增功能。

**首页优化：**

- [x] 三大主模块仍为问事起卦、八字简析、奇门问事。
- [x] 主模块卡片从同质深色大块调整为白底卡片、左侧 accent 轨道和圆形进入箭头。
- [x] 八字 / 奇门使用不同 accent 色，提升扫读时的区分度。
- [x] “感情关系观察”仍位于“常见场景”低优先级区域，不成为第四大主模块。
- [x] 场景入口去掉高权重黑色按钮，改为文本式入口，降低视觉抢占。

**问事结果页优化：**

- [x] 页面标题改为“卦象已生成”，完整原问题移入独立“本次问题”卡片。
- [x] 元信息标签化展示，避免长问题与事项 / 时间信息互相挤压。
- [x] 免费解读正文行距与字号略提升，增强长文本可读性。
- [x] “查看完整解析”按钮层级增强，但仍使用既有 `mock_button` 解锁逻辑。
- [x] 分享区域背景降噪，继续强调长图不含完整原问题或完整解析全文。

**问事分享长图优化：**

- [x] 预览弹窗增加“摘要版”标识，说明长图内容定位。
- [x] Canvas 背景、内层卡片和卦象概览卡片对比度优化。
- [x] 变化观察卡片支持自动换行、序号和更充足行距，避免长句拥挤。
- [x] 本卦 / 变卦名称支持最多两行换行截断，降低溢出风险。
- [x] 长图仍不展示完整原问题、完整解析全文、`session_key`、payload、prompt 或小程序码。
...
- [ ] 首页三大主模块视觉层级清晰，感情关系观察入口低于主模块。
- [ ] 首页主模块卡片文字、标签和箭头在窄屏不重叠。
- [ ] 问事结果页长问题在“本次问题”卡片中可读，不挤压元信息。
- [ ] 免费解读、解锁按钮、分享区域层级清楚。
- [ ] 问事长图生成、预览、保存正常；长句不溢出；卦名不越界。

**结论：** UI1 是小程序 UI 可读性小修；不需要 backend / frontend / SQL / deploy，不上传体验版、不提审。

## 42. Phase HOME2：首页信息架构与视觉层级优化

**范围：** 小程序首页、about 页与相关文档；**不改** backend / frontend / SQL / deploy / `.env*` / 默认算法，不新增真实功能模块。

**首页新信息架构：**

- [x] 顶部品牌区改为短文案：说明这是传统文化学习与行动节奏整理工具，并提供“开始问事 / 查看历史”两个入口。
- [x] 三大主模块仍为问事起卦、八字简析、奇门问事，保持最高视觉层级和原有跳转路径。
- [x] 常见场景区域保留“感情关系观察”，继续跳转 `/pages/ask/ask?scene=relationship`。
- [x] 常见场景中“事业选择 / 学习规划 / 人际沟通”仅展示为“规划中”，不跳转。
- [x] 新增“传统文化小工具”规划区，梦境意象解析、姓名笔画观察、起名灵感助手、节气行动卡均为非点击规划态。
- [x] 底部保留使用边界、历史记录和关于与说明入口。

**视觉优化：**

- [x] 首页背景调整为深墨暖黑，降低金色点缀饱和度。
- [x] 首屏减少大段说明，品牌区更像工具入口。
- [x] 主模块卡片保持浅色高层级，规划项使用低对比 chip / 轻卡片。
- [x] 卡片间距、标题字号和标签数量收敛，降低文案密度。
- [x] 移除首页旧的独立“如何选择模块”大卡片，避免第一屏拥挤。

**about 页同步：**

- [x] “当前已开放”列出问事起卦、八字简析、奇门问事、感情关系观察。
- [x] “规划中”说明未来小工具仍未作为正式功能入口，不承诺上线时间。
- [x] 内容边界改为更克制的否定说明，不提供现实专业服务替代。

**兼容性与合规：**

- [x] 未修改 backend / frontend / SQL / deploy / `.env*`。
- [x] 未新增页面、API、广告、支付、微信登录、手机号授权或算法选择 UI。
- [x] 八字 / 奇门普通创建流程不传内部算法字段。
- [x] 未上线小工具均不可点击，不会跳转空白页。
- [x] 感情关系观察仍复用问事起卦链路，不改变提交 payload。
...
- [ ] 首页第一屏显示正常，品牌区文案清晰。
- [ ] 三大主模块卡片层级最高，文字 / 标签 / 箭头不重叠。
- [ ] 感情关系观察位于常见场景区域，并能进入 relationship scene。
- [ ] 规划中场景和传统文化小工具不会误跳转。
- [ ] 历史入口、关于入口正常。
- [ ] 首页无内部算法选择、广告、支付或登录入口。
- [ ] 首页无强预测或现实结果承诺文案。
- [ ] 页面不拥挤，不白屏。
...
- [x] 梦境意象解析、姓名笔画观察、起名灵感助手、节气行动卡位于 `HOME_TOOL_ITEMS`，均无 `url` 字段。
...
- [x] 规划中说明包含梦境意象解析、姓名笔画观察、起名灵感助手、节气行动卡。
- [x] 未承诺上线时间。
- [x] 未展示内部算法字段或算法选择。
...
- [x] 梦境意象解析、姓名笔画观察、起名灵感助手、节气行动卡均显示“规划中”状态 pill，不可点击，不新增页面或 API。
...
- [x] 截图复核后隐藏历史页页面内冗余“返回”链接，保留微信原生导航返回。
- [x] 二次截图复核后隐藏 about / 问事页页面内冗余“返回”链接，保留微信原生导航返回。
- [x] about 页同步“传统文化观察工具”说法，并轻量统一金色强调色和规划卡片样式。

**兼容性与合规：**

- [x] 未修改 backend / frontend / SQL / deploy / `.env*`。
- [x] 未新增真实梦境 / 姓名 / 起名 / 节气行动卡功能。
