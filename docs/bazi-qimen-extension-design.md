...
### 10.63 Phase UI1 交付说明（小程序首页、结果页、长图可读性优化）

**目标：** 在不改变业务流程、后端接口、默认算法和隐私边界的前提下，优化小程序首页主模块、问事结果页和问事分享长图的视觉层级与长文本可读性。

**实现范围：**

- [x] 首页三大主模块卡片从同质深色块调整为白底卡片 + 左侧 accent 轨道 + 圆形进入箭头
- [x] 首页感情关系观察仍为“常见场景”低优先级入口，不成为第四主模块
- [x] 问事结果页标题改为“卦象已生成”，完整问题移入独立“本次问题”卡片
- [x] 问事结果页元信息标签化，免费解读行距与解锁按钮层级优化
- [x] 问事长图预览增加“摘要版”标识
- [x] 问事 Canvas 长图优化背景、卦象概览卡片、变化观察卡片换行与卦名截断
...
**待人工勾选：** 微信 DevTools / 真机检查首页窄屏卡片、结果页长问题、长图生成 / 预览 / 保存与相册权限流程。

**结论：** UI1 为小程序可读性小修；无需 backend / frontend / SQL / deploy，不上传体验版、不提审。

---

### 10.64 Phase HOME2 交付说明（首页信息架构与视觉层级优化）

**目标：** 在不新增业务功能、不改变默认算法和不修改后端接口的前提下，重排小程序首页信息架构，为后续传统文化小工具扩展预留空间。

**实现范围：**

- [x] 首页顶部品牌区改为短说明 + “开始问事 / 查看历史”，降低首屏说明密度。
...
- [x] 首页整体采用深墨暖黑背景、低饱和金色点缀和浅色主模块卡片。
- [x] 主模块卡片保持最高层级；常见场景与规划区使用更轻的 chip / 轻卡片。
- [x] 未上线项目不跳转，不新增页面和 API。
- [x] 历史、关于、普通问事、八字、奇门入口路径保持不变。
...
**待人工勾选：** 微信 DevTools / 真机检查首页第一屏、三主模块层级、relationship 场景跳转、规划项不可点击、历史 / 关于入口和窄屏文字不重叠。
...
- [x] 传统文化小工具规划区改为两列均衡布局，每项显示“规划中”状态 pill，不可点击
- [x] 底部边界说明、历史记录和关于入口降噪，降低压迫感

**历史页 / about 页：**

- [x] 历史页保留浅色背景，仅统一金色强调色和轻量空状态按钮；截图复核后隐藏页面内冗余返回链接，保留微信原生导航返回
