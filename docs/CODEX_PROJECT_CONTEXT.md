...
- 当前分支：`main`
- 当前 HEAD：`c310cc7 feat(miniprogram): add relationship question scene`
- LOVE1-QA 不改 backend / Web / SQL / deploy / `.env*`，仅做代码层回归与文档记录

LOVE1-QA 代码层结论：

- 首页 `HOME_MODULES` 仍仅包含问事起卦、八字简析、奇门问事三项。
- 感情关系观察入口位于 `HOME_SCENE_ITEMS`，路径为 `/pages/ask/ask?scene=relationship`，不是第四个主模块。
- 问事页 `onLoad(options)` 读取 `scene=relationship` 后切换标题、说明、边界提示、placeholder 和 6 个模板。
- 模板点击只调用 `onTemplateTap` 填充 textarea，不调用 `submitForm`，不会自动提交。
- `createDivination` 仍只提交 `session_key`、`category_id`、`question`、`confirm_disclaimer`，不新增 `scene`、`algorithm_version` 或其他后端字段。
- 历史列表继续不展示完整原问题；问事长图继续使用 `QUESTION_SUMMARY` 摘要，不展示完整原问题。
