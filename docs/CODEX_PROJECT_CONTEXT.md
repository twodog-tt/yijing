...
- 如何选择模块：场景到模块映射。
- 常见场景：LOVE1 后新增“感情关系观察”低优先级入口，跳转到问事起卦场景页；首页仍保持三模块为主。
...
- 当前分支：`main`
- 当前 HEAD：`aaaa888 docs(miniprogram): record relationship scene QA results`
- UI1 不改 backend / Web / SQL / deploy / `.env*`，不新增功能，不改变默认算法

UI1 代码层结论：

- 首页主模块卡片改为白底、左侧 accent 和圆形箭头，三模块入口更易扫读；感情关系观察仍保持低优先级场景入口。
- 问事结果页不再把完整问题作为超大页面标题，改为“卦象已生成”页头 + 独立“本次问题”卡片，提升长问题阅读性。
- 问事结果页元信息改为标签化展示，免费解读正文行距和解锁按钮层级优化。
- 问事分享长图预览增加“摘要版”标识；Canvas 长图的变化观察卡片支持自动换行、序号和更充足行距，卦名支持换行截断。
- 长图仍不展示完整原问题、完整解析全文、`session_key`、payload、prompt 或小程序码。
