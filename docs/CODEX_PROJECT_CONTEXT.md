...
更新时间：2026-06-29
任务：HOME2-UI-POLISH：首页视觉层级与可读性精修
...
- 顶部品牌区：小标题为“传统文化观察工具”，主标题“把问题整理清楚”，提供“开始问事 / 查看历史”入口。
- 三大主模块：问事起卦、八字简析、奇门问事，保持最高视觉层级。
- 常见场景：感情关系观察为唯一已上线场景入口，跳转 `/pages/ask/ask?scene=relationship`；事业选择、学习规划、人际沟通仅展示“规划中”。
- 传统文化小工具：梦境意象解析、姓名笔画观察、起名灵感助手、节气行动卡仅为规划展示，两列均衡布局，不可点击，不新增页面或 API。
- 使用边界说明：不承诺具体结果、不替代现实决策、不提供投资医疗法律等专业建议。
- 保留历史记录、关于与说明入口。

HOME2-UI-POLISH 已按截图反馈精修首屏密度、主模块卡片高度、规划态 disabled 视觉、小工具两列布局，以及历史页 / about 页轻量统一；仍需维护者在微信 DevTools / 真机重新截图复核。
...
- HOME2-QA：首页结构、三大主模块、relationship 场景、规划态不可点击、about 同步和合规检查代码层通过；仍需 DevTools / 真机本地勾选。
- HOME2-UI-POLISH：基于截图反馈精修首页首屏、主模块卡片、常见场景、传统文化小工具规划区、底部区域，并轻量统一历史页 / about 页；不改后端、Web、SQL、默认算法。
...
- HOME2-QA：仅文档记录与代码层验收；未修改 backend / frontend / SQL / deploy / `.env*`。
- HOME2-UI-POLISH：仅小程序首页 / 历史页 / about 样式与文案小修、文档记录；不新增功能、不执行 SQL、不上传体验版。
- W1 / W2：Web 八字 / 奇门同步与 Web 首页对齐。

## 12. 当前最新 commit 线索

本次 HOME2-UI-POLISH 开始时：

- 当前分支：`main`
- 当前 HEAD：`63b9ecb docs(miniprogram): record home architecture QA results`
- HOME2-UI-POLISH 不改 backend / Web / SQL / deploy / `.env*`，不新增真实功能模块，不改变默认算法

HOME2-UI-POLISH 代码层结论：

- hero 小标题改为“传统文化观察工具”，移除额外说明行，减少第一屏重复和密度。
- “开始问事 / 查看历史”路径保持正确。
...
- 梦境意象解析、姓名笔画观察、起名灵感助手、节气行动卡仍为规划态，无 `url`，小工具区改为两列均衡布局，并使用“规划中”状态 pill。
