...
HOME2-QA 代码层已通过；仍需维护者在微信 DevTools / 真机勾选首页渲染、relationship 跳转、规划项不可点击和窄屏不重叠。
...
- 当前分支：`main`
- 当前 HEAD：`28a713e feat(miniprogram): refine home information architecture`
- HOME2-QA 不改 backend / Web / SQL / deploy / `.env*`，不新增真实功能模块，不改变默认算法

HOME2-QA 代码层结论：

- 顶部品牌区短文案、“开始问事 / 查看历史”路径存在且正确。
- 三大主模块仍为问事起卦、八字简析、奇门问事，路径不变。
- 感情关系观察仍位于 `HOME_SCENE_ITEMS`，路径为 `/pages/ask/ask?scene=relationship`，不是第四个主模块。
- 事业选择、学习规划、人际沟通位于 `HOME_PLANNED_SCENE_ITEMS`，均无 `url`。
- 梦境意象解析、姓名笔画观察、起名灵感助手、感情签位于 `HOME_TOOL_ITEMS`，均无 `url`。
- 首页 WXML 中规划项使用普通 `view`，未做成可点击 `navigator`。
- about 页已同步“当前已开放 / 规划中”说明，不承诺上线时间。
- 自动化检查：miniprogram static、release privacy、API smoke、全量 JS `node --check`、`git diff --check` 均通过。
