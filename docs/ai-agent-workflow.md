...
## Phase RC1（Web + 小程序全链路回归）

**目标：** 不开发新大功能，只做全链路回归、体验小修、文案统一、错误提示统一、隐私与合规检查。

**允许修改：** `miniprogram/`、`frontend/`、`docs/`；仅明确 bug 时可小范围改 `backend/internal/handler/`、`backend/internal/service/`。

**禁止：** 新模块、真实广告、支付、微信登录、手机号授权、新 SQL、重构后端、改 DeepSeek 逻辑、改部署配置。

**RC1 收口要点（2025-06）：**

- 八字 / 奇门完整报告：生产 UI 使用 `free_unlock`，不出现 mock 广告 /「观看视频」文案
- Web `frontend/lib/api.ts` 统一业务错误映射，避免暴露英文后端原始 message
- 小程序 / Web 完整报告相关错误文案统一为「完整报告获取失败」等克制表述
- 历史记录页当前仅聚合六爻；八字 / 奇门在各自表单页「最近记录」查看（合并历史为后续阶段）
